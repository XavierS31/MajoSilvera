...
export async function requireAdmin(event: ApiEvent): Promise<AdminClaims> {
  ...
  const url = process.env.OAUTH2_TOKEN_INTROSPECT_URL
  ...
  try {
    response = await fetch(url, { method: 'POST', headers: { authorization: `Basic ${Buffer.from(`${clientId}:${secret}`).toString('base64')}`, 'content-type': 'application/x-www-form-urlencoded' }, body: new URLSearchParams({ token }).toString() })
  ...
  if (!adminEmail || claims.email.toLowerCase() !== adminEmail.toLowerCase()) throw new Error('FORBIDDEN')
