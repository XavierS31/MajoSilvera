...
export function Navbar() {
  ...
  return <nav className={`nav ${scrolled || pathname !== '/' ? 'is-solid' : ''}`} aria-label="Navegación principal">
    <div className="nav-inner">
      ...
      <div className="nav-links">{navLinks.filter((link) => link.to !== '/contacto').map((link) => <Link key={link.to} to={link.to}>{link.label}</Link>)}<Link className="button-gold" to="/contacto">Contacto</Link></div>
      <button className="menu-toggle" type="button" aria-expanded={open} aria-label="Abrir menú" onClick={() => setOpen((value) => !value)}>☰</button>
    </div>
    <div className={`mobile-nav ${open ? 'open' : ''}`}>{navLinks.filter((link) => link.to !== '/contacto').map((link) => <Link key={link.to} to={link.to} onClick={() => setOpen(false)}>{link.label}</Link>)}<Link className="button-gold" to="/contacto" onClick={() => setOpen(false)}>Contacto</Link></div>
  </nav>
}
